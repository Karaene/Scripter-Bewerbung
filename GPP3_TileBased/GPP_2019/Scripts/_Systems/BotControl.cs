...
namespace GPP_2019
{
    class BotControl : ISystem
    {
        ...
        private double GetDistance(Vector2D enemy_pos, Vector2D player)
        {
            double templength = Math.Pow(enemy_pos.X- player.X,2) + Math.Pow(enemy_pos.Y - player.Y, 2);
            double length = Math.Sqrt(templength);
            return length;
        }
        ...
        public void Update(double deltaTime)
        {
            ...
            foreach (var bot in _activeBots)
            {
                if (bot.GameObject.Active)
                {
                    ...
                    double length = GetDistance(PlayerControl.Instance.Player.GameObject.Transform.Position, bot.GameObject.Transform.Position);
                    ...
                    tempPostion = bot.GameObject.Transform.Position;
                    ...
                    if (length <300) {
                        if ((Math.Abs(botScreenPosition.X - playerScreenPosition.X) > ALLOWED_ENEMY_PLAYER_DISTANCE ||
                            Math.Abs(botScreenPosition.Y - playerScreenPosition.Y) > ALLOWED_ENEMY_PLAYER_DISTANCE) && !bot.GameObject.GetComponent<BoxColliderComponent>().HitCollider && PlayerControl.Instance.Player.GameObject.Active)
                        {
                            ...
                            //if its going to collide change it back
                            if (PhysicSystem.Instance.CheckEnemyCollision(bot.GameObject)) {
                                bot.GameObject.Transform.Position = tempPostion;
                            }
                        }
                    }
                    ...
                    else if (bot.GameObject.GetComponent<BoxColliderComponent>().HitCollider)
                    {
                        ...
                        bot.GameObject.GetComponent<SpriteRendererComponent>().GameObject.Transform.Position = bot.GameObject.Transform.Position;
                    }
                        /*
