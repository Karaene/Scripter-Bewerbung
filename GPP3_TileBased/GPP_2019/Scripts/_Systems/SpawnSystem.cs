...
namespace GPP_2019
{
    class SpawnSystem : ISystem
    {
        ...
        /*                 EnemyType       EnemyPool   */
        public Dictionary<EnemyType, List<GameObject>> _enemyPools = new Dictionary<EnemyType, List<GameObject>>();
        ...
        public void Init()
        {
            ...
            for (int i = 0; i < 100; i++)
            {
                ...
                enemy.Transform.Size = new Size(58, 58);
                enemy.AddComponent(RenderSystem.Instance.CreateSpriteRenderer("Sprites/enemy.png", Layer.MIDDLEGROUND));
                enemy.AddComponent(PhysicSystem.Instance.CreateBoxCollider(enemy, -10, -10));
        ...
        public void SpawEnemyHorde(SpawnerComponent spawner, EnemyType enemyType)
        {
            ...
            foreach (var enemy in _enemyPools[enemyType])
            {
                if (!enemy.Active)
                {
                    ...
                    distanceBetween += new Vector2D(enemy.Transform.Size.Width, 0);
                    --spawner.SpawnAmount;
                    enemy.Active = true;
        ...
        public void DeactivateEverySpawner()
        {
            foreach (var spawner in _activeSpawner)
            {
                if (spawner.GameObject.Active)
                    spawner.GameObject.Active = false;
            }
        }

        public void ActivateEverySpawner()
        {
            foreach (var spawner in _activeSpawner)
            {
                if (!spawner.GameObject.Active)
                    spawner.GameObject.Active = true;
            }
        }
